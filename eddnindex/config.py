...
class Config(object):
    ...
    def __init__(self, config_filename: str, override_config_filename: Union[str, None] = None):
        config = configparser.ConfigParser(interpolation=configparser.ExtendedInterpolation())

        app_config = os.path.join(os.path.dirname(sys.argv[0]), config_filename)
        ...
        config.read([app_config, site_config, user_config, override_config_filename])
        ...
        self.edsm_systems_file = edsm.get(
                ...
                os.path.join(self.edsm_dump_dir, 'systemsWithCoordinates.jsonl.bz2')
        )

        self.edsm_systems_without_coords_file = edsm.get(
                'SystemsWithoutCoordinates',
                os.path.join(self.edsm_dump_dir, 'systemsWithoutCoordinates.jsonl.bz2')
        )

        self.edsm_systems_without_coords_pre_purge_file = edsm.get(
                'SystemsWithoutCoordinatesPrePurge',
                os.path.join(self.edsm_dump_dir, 'systemsWithoutCoordinates-2020-09-30.jsonl.bz2')
        )

        self.edsm_hidden_systems_file = edsm.get(
                'HiddenSystems',
                os.path.join(self.edsm_dump_dir, 'hiddenSystems.jsonl.bz2')
        )

        self.edsm_bodies_file = edsm.get(
                'Bodies',
                os.path.join(self.edsm_dump_dir, 'bodies.jsonl.bz2')
        )

        self.edsm_stations_file = edsm.get(
                'Stations',
                os.path.join(self.edsm_dump_dir, 'stations.json.gz')
        )

        self.eddb_systems_file = eddb.get(
                'Systems',
                os.path.join(self.eddb_dir, 'systems.csv.bz2')
        )

        self.eddb_stations_file = eddb.get(
                'Stations',
                os.path.join(self.eddb_dir, 'stations.jsonl')
        )

        self.edsm_systems_cache_file = cache.get(
                'EDSMSystems',
                os.path.join(self.cache_dir, 'edsmsys-index-update-syscache.bin')
        )

        self.edsm_bodies_cache_file = cache.get(
                'EDSMBodies',
                os.path.join(self.cache_dir, 'edsmbody-index-update-bodycache.bin')
        )

        self.eddn_reject_dir = rejects.get(
                'EDDN',
                os.path.join(self.output_dir, 'eddn-index-update-reject')
        )

        self.edsm_systems_reject_file = rejects.get(
                'EDSMSystems',
                os.path.join(self.output_dir, 'edsmsys-index-update-reject.jsonl')
        )

        self.edsm_bodies_reject_file = rejects.get(
                'EDSMBodies',
                os.path.join(self.output_dir, 'edsmbodies-index-update-reject.jsonl')
        )

        self.edsm_stations_reject_file = rejects.get(
                'EDSMStations',
                os.path.join(self.output_dir, 'edsmstations-index-update-reject.jsonl')
        )

        self.eddb_systems_reject_file = rejects.get(
                'EDDBSystems',
                os.path.join(self.output_dir, 'eddbsys-index-update-reject.jsonl')
        )

        self.eddb_stations_reject_file = rejects.get(
                'EDDBStations',
                os.path.join(self.output_dir, 'eddbstations-index-update-reject.jsonl')
        )
